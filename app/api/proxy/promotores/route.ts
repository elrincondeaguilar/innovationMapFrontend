...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Para DELETE, el backend puede retornar vacío o un mensaje
    const data = response.status === 204 ? { message: "Deleted successfully" } : await response.json();
    return NextResponse.json(data);
  } catch (error) {
    console.error("Promotores DELETE proxy error:", error);
    return NextResponse.json(
      {
        error: "Failed to connect to backend",
        details: error instanceof Error ? error.message : "Unknown error",
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error("Promotores PUT proxy error:", error);
    return NextResponse.json(
      {
        error: "Failed to connect to backend",
        details: error instanceof Error ? error.message : "Unknown error",
